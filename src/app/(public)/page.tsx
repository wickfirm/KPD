...
export default async function HomePage() {
  ...
  return <div className="development-pdf-home" id="top">
    ...
    <section className="pdf-section pdf-events-grid" aria-label="Blogs and updates">{articles.map((article) => <article className="pdf-event-card" key={article.slug}>{article.coverImage ? <img src={article.coverImage} alt={article.coverImageAlt || article.title} /> : null}<h3>{article.title}</h3><p>{article.summary}</p><Link href={`/legacy/news-article.html?slug=${encodeURIComponent(article.slug)}`}>Explore</Link></article>)}</section>
  </div>;
}
